...
def _llm_eval(prompt: str, max_tokens: int = 500) -> dict:
    """Call GPT-4o-mini for evaluation. Returns parsed JSON with 'score' and 'reason'."""
...
def evaluate_single(
    ...
    # Run the 4 independent metrics (groundedness, faithfulness, context_relevance, answer_relevance)
...
def run_evaluation_pipeline(
    ...
    # Refusal rate: answers that abstained
    refusals = [
        r for r in successful
        if r.get("chosen_source", "").startswith("none")
        or "could not find" in r.get("answer_preview", "").lower()
        or "لم أتمكن" in r.get("answer_preview", "")
    ]
